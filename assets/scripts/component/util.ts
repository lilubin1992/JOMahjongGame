...
export function createBreathAction(node:Node, min = 0.9, max = 1.1)
{
    
    // const action = repeatForever(sequence(scaleTo(0.6, max), scaleTo(0.6, min)));
    // node.runAction(action);
}

export function destroyBreathAction(node:Node)
{
    // node.stopAllActions();
}